import webbrowser
import threading
from flask import Flask, render_template, request, redirect, url_for, flash, send_file, current_app, session
import os
import io
...
# Add this after your imports - NEW DATABASE FUNCTIONS
import pymssql
...
def get_db_connection() -> Tuple['pymssql.Connection', 'pymssql.Cursor']:
    """Minimal context manager for Azure SQL connections using pymssql"""
    ...
    try:
        # BASIC pymssql connection - NO SSL PARAMETERS
        connection = pymssql.connect(
            host=config['server'],  # Required: Azure SQL server
            user=config['username'],  # Required: username@server format
            password=config['password'],  # Required: SQL password
            database=config['database'],  # Required: database name
            port=config['port'],  # Required: usually 1433
            login_timeout=30,  # Optional: connection timeout
            timeout=60  # Optional: query timeout
        )
        ...
        current_app.logger.info(f"✅ Connected to {config['database']} successfully")
...
def dashboard():
    ...
    try:
        ...
        return render_template('error.html', error="Dashboard unavailable", error_code=503)
...
def dictfetchall(cursor):
    """Fetch all rows as dictionaries - FIXED VERSION"""
    # Get columns BEFORE any fetch operations
    ...
    for row in rows:
        if len(columns) == len(row):
            ...
            row_dict = {}
            for i, col in enumerate(columns[:len(row)]):
                row_dict[col] = row[i]
...
@app.route('/trusts')
def trusts_list():
    """Safe trusts listing - uses SELECT * to avoid schema issues"""
    ...
    try:
        with get_db_connection() as (conn, cursor):
            ...
            if mode == 'capture':
                cursor.execute("""
                    SELECT * FROM Trusts 
                    WHERE RecordStatus NOT IN ('0000 - Imported', '0002 - Edited')
                    ORDER BY TrustName
                """)
                ...
                # Default - all trusts
                cursor.execute("SELECT * FROM Trusts ORDER BY TrustName")

            # Get actual column names from the result
            if cursor.description:
                columns = [desc[0] for desc in cursor.description]
            else:
                columns = ['TrustID', 'TrustName', 'RecordStatus']  # Fallback
            ...
            for row in all_rows:
                if len(columns) >= len(row):
                    # Normal case
                    trust_dict = dict(zip(columns, row))
                else:
                    # Handle extra columns
                    trust_dict = {}
                    for i, col in enumerate(columns):
                        trust_dict[col] = row[i] if i < len(row) else None
                    # Add extra columns if they exist
                    for i in range(len(columns), len(row)):
                        trust_dict[f'ExtraCol_{i}'] = row[i]

                # Ensure we have essential fields
                trust_dict.setdefault('TrustID', trust_dict.get('TrustID') or trust_dict.get('trustid'))
                trust_dict.setdefault('TrustName', trust_dict.get('TrustName') or trust_dict.get('trustname'))
                trust_dict.setdefault('RecordStatus', trust_dict.get('RecordStatus') or trust_dict.get('recordstatus'))

                trusts.append(trust_dict)

            app.logger.info(f"✅ Loaded {len(trusts)} trusts for mode '{mode}'")
            app.logger.info(f"   Sample columns: {columns[:5]}...")  # Log first 5 columns
        ...
        return render_template('trusts.html',
                               ...
                               page_title=f"Trusts - {mode.title()}")

    except Exception as e:
        app.logger.error(f"❌ Error in trusts_list({mode}): {e}")
        app.logger.error(f"   Full traceback: {str(e)}")

        # Return diagnostic info instead of template error
        return f"""
        <html>
            <head><title>Trusts Debug - {mode}</title></head>
            <body style="font-family: Arial, sans-serif; padding: 20px;">
                <h1>🔍 Trusts Debug Mode: {mode}</h1>
                <h2>Database Connection: ✅ Working</h2>
                <p><strong>Error:</strong> {str(e)}</p>
                <h3>SQL Schema Issue</h3>
                <p>The query failed due to missing columns in your Trusts table.</p>
                <p><a href="/debug/schema" class="btn">🔧 Show Table Schema</a></p>
                <p><a href="/" class="btn">🏠 Home</a></p>
                <style>
                    .btn {{ 
                        display: inline-block; padding: 10px 20px; 
                        margin: 5px; background: #3498db; color: white; 
...
def debug_schema():
    """Temporary route to show actual table schema"""
    try:
        with get_db_connection() as (conn, cursor):
            # Get Trusts table structure
            cursor.execute("""
            ...
            # Get sample data
            cursor.execute("SELECT TOP 3 * FROM Trusts")
            sample_data = cursor.fetchall()
            sample_columns = [desc[0] for desc in cursor.description] if cursor.description else []

            return render_template('debug_schema.html',
                                   columns_info=columns_info,
                                   sample_data=sample_data,
                                   sample_columns=sample_columns)
    except Exception as e:
        return f"<pre>Schema debug failed: {e}</pre>"
...
def generate_file_content(trust_id, gh_unique_id):
    with get_db_connection() as (conn, cursor):
        ...
        lines = []
        gh_line = (
            f"{hgh['SectionIdentifier']}|{hgh['HeaderType']}|{hgh['MessageCreateDate']}|{hgh['FileLayoutVersion']}|"

...
def validate_trust(trust):
    ...
    if trust['PostalSameAsPhysical']:
        # If same, postal must be blank
        postal_fields = ['PostalAddressLine1', 'PostalAddressLine2', 'PostalAddressLine3', 'PostalAddressLine4', 'PostalCode']
        for field in postal_fields:
            if sanitize(trust.get(field)):
                errors.append(f"Postal field {field} must be blank if same as physical.")
    else:
        if not trust['PostalAddressLine1']:
            errors.append("Postal Address Line 1 mandatory if not same as physical.")
        if not trust['PostalCode']:
            errors.append("Postal Code mandatory if postal address provided.")
    if not trust['PhysicalStreet'] and not trust['PostalAddressLine1']:
        errors.append("At least one full address (physical or postal) must be provided.")
    if trust['PhysicalStreet'] and not trust['PhysicalPostalCode']:
        errors.append("Physical Postal Code mandatory if street provided.")
    # Nature must be valid code
    if trust['NatureOfPerson'] not in ['PARTNERSHIP', 'FOREIGN_COMPANY', 'OTHER_CO', 'INTERVIVOS_TRUST', 'GOVERNMENT_ENTITY', 'UNINCORPORATED_BODY_OF_PERSONS', 'ASSOC_NOT_FOR_GAIN']:
        errors.append("Invalid Nature of Person for trust.")
    return errors

def validate_beneficiary(beneficiary):
    errors = []
    is_natural = beneficiary['IsNaturalPerson']  # Field 410
    if is_natural:
        if not beneficiary['LastName']:
            errors.append("Surname mandatory for natural person.")
        if not beneficiary['FirstName']:
            errors.append("First Name mandatory for natural person.")
        if not beneficiary['DateOfBirth']:
            errors.append("DOB mandatory for natural person.")
        if not beneficiary['IDNumber'] and not beneficiary['PassportNumber']:
            errors.append("ID or Passport mandatory for natural person.")
        if beneficiary['PassportNumber'] and not beneficiary['PassportCountry']:
            errors.append("Passport Country mandatory if passport provided.")
        # Company fields must be blank
        if beneficiary['CompanyIncomeTaxRefNo'] or beneficiary['CompanyRegistrationNumber'] or beneficiary['CompanyRegisteredName']:
            errors.append("Company fields must be blank for natural person.")
    else:
        if not beneficiary['CompanyIncomeTaxRefNo']:
            errors.append("Company Tax Ref mandatory for non-natural.")
        if not beneficiary['CompanyRegistrationNumber']:
            errors.append("Company Reg No mandatory for non-natural.")
        if not beneficiary['CompanyRegisteredName']:
            errors.append("Company Registered Name mandatory for non-natural.")
        # Individual fields must be blank
        individual_fields = ['LastName', 'FirstName', 'OtherName', 'Initials', 'DateOfBirth', 'IDNumber', 'PassportNumber', 'PassportCountry']
        for field in individual_fields:
            if sanitize(beneficiary.get(field)):
                errors.append(f"Individual field {field} must be blank for non-natural.")
    # Address (same as trust logic)
    if beneficiary['PostalSameAsPhysical']:
        postal_fields = ['PostalAddressLine1', 'PostalAddressLine2', 'PostalAddressLine3', 'PostalAddressLine4', 'PostalCode']
        for field in postal_fields:
            if sanitize(beneficiary.get(field)):
                errors.append(f"Postal field {field} must be blank if same as physical.")
    else:
        if not beneficiary['PostalAddressLine1']:
            errors.append("Postal Line 1 mandatory if not same.")
        if not beneficiary['PostalCode']:
            errors.append("Postal Code mandatory if postal provided.")
    if not beneficiary['PhysicalStreet'] and not beneficiary['PostalAddressLine1']:
        errors.append("At least one full address must be provided.")
    return errors

def validate_transactions(transactions):
    errors = []
    foreign_req_codes = ['0192', '0193', '4216', '4217', '4218', '4220', '4221', '4252', '4253', '4276']
    for tad in transactions.get('TAD', []):
        if tad['Code'] in foreign_req_codes:
            credits = sanitize(tad.get('ForeignTaxCredits'))
            if not credits or float(credits) < 0:
                errors.append(f"Foreign Tax Credits must be >= 0 for code {tad['Code']} (blank invalid).")
    return errors
...
def trusts_list():
    ...
    if mode == 'submissions':
        ...
        cursor.execute("SELECT * FROM Trusts ORDER BY RecordStatus DESC")
        mode_out = 'submissions'
    else:
        cursor.execute(
            "SELECT * FROM Trusts WHERE RecordStatus NOT IN ('9010 - SUBMITTED TO SARS','9999 - Uploaded and Processed Successfully') ORDER BY RecordStatus DESC")
        mode_out = 'capture'
...
def generate_report():
    ...
    query = """
        SELECT a1.trustid, a1.trustname, a1.recordstatus, COUNT(m1.trustid) AS beneficiary_count
        FROM trusts AS a1 
        INNER JOIN [dbo].[Beneficiaries] AS m1 ON m1.trustid = a1.trustid
        WHERE a1.recordstatus NOT IN ('0000 - IMPORTED', '0002 - Edited')
        GROUP BY a1.trustid, a1.trustname, a1.recordstatus
        ORDER BY a1.trustid
        """
    ...
    timestamp = datetime.now().strftime("%y%m%d%H%M")  # CCYYMMDDHHII format
    filename = f"Completed_Trusts_{timestamp}.csv"
...
def generate_file_content(trust_id, gh_unique_id):
    ...
    # Validate data
    trust_errors = validate_trust(trust)
    if trust_errors:
        raise ValueError("Trust validation failed: " + "; ".join(trust_errors))

    beneficiary_errors = {}
    transactions = {}
    for beneficiary in beneficiaries:
        cursor.execute('SELECT * FROM BeneficiaryTFF WHERE BeneficiaryID = ?', (beneficiary['BeneficiaryID'],))
        tff_data = dictfetchall(cursor)

        ben_errors = validate_beneficiary(beneficiary)
        if ben_errors:
            beneficiary_errors[beneficiary['BeneficiaryID']] = ben_errors
        else:
            # Group transactions by BeneficiaryID
            transactions[beneficiary['BeneficiaryID']] = {
                'TAD': [tad for tad in tad_records if tad['BeneficiaryID'] == beneficiary['BeneficiaryID']],
                'TFF': [tff for tff in tff_data if tff['BeneficiaryID'] == beneficiary['BeneficiaryID']]
            }

    if beneficiary_errors:
        raise ValueError("Beneficiary validation failed: " + "; ".join(
            [f"ID {k}: {'; '.join(v)}" for k, v in beneficiary_errors.items()]))

    transaction_errors = validate_transactions(transactions)
    if transaction_errors:
        raise ValueError("Transaction validation failed: " + "; ".join(transaction_errors))
